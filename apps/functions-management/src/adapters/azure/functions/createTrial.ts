...
export const storeNewTrial: (
  ...
  pipe(
    ...
    Trial.decode,
    E.mapLeft(
      errs => new H.HttpError(`Cannot decode Trial model: [${errs.join("|")}]`)
    ),
    TE.fromEither,
    TE.chain(model =>
      pipe(
        ...
        TE.mapLeft(
          err =>
            new H.HttpError(`An error occurred saving Trial: [${String(err)}]`)
        )
      )
    )
  );
...
> = H.of((req: H.HttpRequest) =>
  pipe(
    ...
    RTE.map(savedTrial =>
      H.successJson({ trialId: savedTrial.id, isEnabled: savedTrial.isEnabled })
    )
  )
);
